...
class LocalModelProvider(BaseLLMProvider):
    ...
    async def generate_stream(
        ...
        **kwargs
        ...
        """Stream tokens as they're generated from the local model."""
        if not state.model or not state.processor:
            raise RuntimeError("Local model is not loaded!")
        
        logging.debug(f"[DEBUG] LocalModel Stream - image_base64 provided: {image_base64 is not None}")
        ...
        try:
            ...
            logging.warning("[LocalModel] TextIteratorStreamer not available, falling back to non-streaming")
            result = await self.generate(message, history, image_base64, **kwargs)
            if result:
                yield result
        except Exception as e:
            logging.error(f"[LocalModel] Streaming error: {e}, falling back to non-streaming")
            result = await self.generate(message, history, image_base64, **kwargs)
            if result:
                yield result
